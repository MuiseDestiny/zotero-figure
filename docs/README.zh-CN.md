...
Zotero Figure 在本机渲染 PDF 页面，使用 DocLayout-YOLO 检测图片、表格及其图注，并在 PDF 阅读器的专用左侧栏中展示裁取结果。PDF 页面和模型推理数据不会发送到远程服务。
...
- 在 Zotero 数据目录中，以 JSON 保存每个附件的结果索引，并以 PNG 保存裁取的图表。
- 重复解析时可按页替换本地旧结果，或保留已有结果并仅补充缺失项。
- 可取消长时间分析；正在推理时取消会终止当前 Worker，后续任务再重建。检测始终使用 640 像素渲染；每个有结果的页面只进行一次高清渲染，最高 4 倍、1200 万像素且单边不超过 8192 像素，再从这张页面图像中裁出所有检测框。
- 无可提取文本的页面也会进入检测，而不是被跳过；有文本的页面会使用完整的英文、中文、意大利文和俄文图表提示词来缩小检测范围。
- 以 PDF 阅读器左侧栏中的插件图标作为唯一主入口；顶部阅读器工具栏不再添加插件按钮。
- 通过自适应宽度的卡片浏览结果，并按“全部”“图”“表”的顺序筛选；每个筛选项同时显示对应结果数量。本地 PNG 会在接近视口时通过可撤销的 Blob URL 懒加载，同时最多读取三张图片。
- 长图注默认折叠，点击图注即可展开或再次折叠。
- 直接在侧栏工具条中解析、取消、将全部结果加入笔记、刷新或清空本地结果。清空后侧栏立即变空，JSON 和 PNG 文件在后台删除，已有 Zotero 标注镜像不受影响。
- 如果安装了 Zotero PDF Translate，可在工具条一键翻译图表说明，再次点击恢复原文；成功译文会按目标语言在本地缓存并复用。
- 可通过工具条按需将已有本地结果转换为 Zotero 标注镜像。
- 每张卡片都可复制或保存图片、跳转到所在页、添加到笔记或删除。
- 可选把结果镜像为带有 `Figure N` 或 `Table N` 标签的 Zotero 原生图片标注，并将图注写入评论；该选项默认关闭。
- 可把单张或全部图表写入 Zotero 笔记，图片作为真实的嵌入式 PNG 附件保存；即使没有创建标注镜像，点击笔记图片仍可跳转到源 PDF 中对应的裁图位置。
...
Zotero Figure 只内置一个模型：优化版 Q8 `optimized-q8-d5d1e664`，大小 20,552,482 字节，SHA-256 为 `d5d1e664fbe639e716be7011aa7493b40f4ab498374b2157988c2ea41bf70daf`。插件会把它释放到 Zotero 数据目录的 `zotero-figure/models/`，并在分析前校验本地副本。
...
3. 点击侧栏工具条中的“解析图和表”，等待页面渲染和检测完成。
4. 在“全部”“图”“表”之间切换，并通过卡片菜单复制或保存图片、跳转到所在页、添加到笔记或删除。
5. 如果安装了 Zotero PDF Translate，可点击语言按钮翻译图表说明，再次点击恢复原文。
...
侧栏始终读取插件自己的本地结果仓库。每个 PDF 附件的元数据位于 Zotero 数据目录下的 `zotero-figure/results/<libraryID>/<attachmentKey>/manifest.json`，PNG 位于相邻的 `images/` 目录。schema v3 的 manifest 按目标语言保存图注译文，并记录带版本的图片缓存身份。重复分析时，只有附件源指纹（文件大小和修改时间；无法取得时回退为 Zotero 条目版本）、结果的分析版本、模型哈希、预览版本和结果指纹都相同，且对应 PNG 仍存在，插件才会直接复用该 PNG。清单、翻译缓存和 PNG 都不使用 Zotero Sync。开启标注同步后，插件会额外创建一份原生标注镜像，用于同步和兼容其他读取标注的插件，但侧栏数据源不会改变。写入笔记的图片是嵌入式附件，不是指向本地 PNG 路径的链接；即使关闭标注镜像，点击笔记图片仍可跳转到源 PDF 中对应的裁图位置。

阅读器空闲时会提前准备模型和 Worker；如果首次解析在准备完成前启动，仍可能较慢。耗时取决于 PDF 页数、页面复杂度、CPU 和可用内存。

解析期间最多同时调度三页。各个有界阶段分别限制为：最多两页进入检测、一个检测页面渲染、一个高清预览页面和一个存储操作。检测页面按 640 像素渲染。对于缓存未命中的结果页，PDF 页面只渲染一次，缩放不超过 4 倍、总像素不超过 1200 万且单边不超过 8192 像素；随后从这张页面图像中裁出所有结果区域。插件复用一个推理 Worker；设备条件允许时，它最多使用四个 WASM 线程，同时为 Zotero 预留逻辑处理能力。取消正在执行的推理会终止 Worker，避免其继续占用 CPU；后续工作再重建 Worker。复用单个 Worker 也避免了重复创建模型会话。

## 隐私

检测由 Zotero 内的 Transformers.js 和 ONNX Runtime Web 完成。插件不会把 PDF 页面发送到远程推理服务，也不会在运行时联网下载模型。
...
| `src/features`        | 阅读器界面与偏好设置                       |
| `src/services`        | 页面分析、推理队列、图片裁取与本地结果仓库 |
