...
- PDF Reader で選択した画像注釈を右クリックすると、ローカルの図表解析結果に追加できます。既存の Figure、Table、Formula タグは保持されます。
...
- 比較レイアウトは Zotero データディレクトリ内の `zotero-figure/comparison-layouts.json` にローカル保存され、大きな比較表にも対応します。既存のレイアウトは自動移行されます。バックアップにはこのファイルも含めてください。レイアウトは Zotero Sync では同期されません。
...
- **一括処理:** ライブラリアイテムまたは PDF 添付ファイルを選択して右クリックし、`PDF Figure >` から解析、ノートへの追加、注釈の作成を実行します。画像注釈を選択し、`Figure に追加` を選ぶと一括インポートできます。
