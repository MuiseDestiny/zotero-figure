menu-analyze = 解析 PDF 图、表和公式
menu-cancel-analysis = 取消版面分析
menu-view-figures = 仅显示生成的图表
menu-view-annotations = 仅显示普通标注
menu-add-note = 将图表添加到笔记
menu-remove-generated = 删除生成的图表标注
menu-remove-all = 删除全部标注
reader-menu-add-to-figure = 添加到 Figure
progress-add-to-results = 正在将图片标注添加到图表解析结果…
progress-add-to-results-done = 已添加 { $created } 个图片标注（{ $skipped } 个已存在）

batch-menu-analyze = 解析图、表和公式
batch-menu-analyze-note = 解析图、表和公式并写入笔记
batch-menu-analyze-annotations = 解析图、表和公式并写入标注
batch-progress-no-annotations = 未选择图片标注
batch-progress-import-done = 已导入 { $created } 个图片标注（{ $skipped } 个已存在），失败 { $failed } 个
batch-progress-no-pdf = 所选条目中没有可用的 PDF 附件
batch-progress-already-running = 已有 PDF Figure 批处理正在运行
batch-progress-current = [{ $current }/{ $total }] { $title }：{ $detail }
batch-progress-finished-item = 已完成第 { $current }/{ $total } 个 PDF
batch-progress-done = 已完成 { $succeeded } 个 PDF，失败 { $failed } 个，共找到 { $results } 个结果，创建 { $notes } 篇笔记和 { $annotations } 个标注

gallery-title = PDF 图表库
gallery-menu-open = PDF 图表库
gallery-refresh =
    .title = 刷新资料库索引
    .aria-label = 刷新资料库索引
gallery-reset-filters =
    .title = 重置筛选和搜索
    .aria-label = 重置筛选和搜索
gallery-toolbar-collapse =
    .title = 收起筛选工具
    .aria-label = 收起筛选工具
gallery-toolbar-expand =
    .title = 展开全部筛选
    .aria-label = 展开全部筛选
gallery-view-switch-to-documents =
    .title = 切换到文献分栏
    .aria-label = 切换到文献分栏
gallery-view-switch-to-waterfall =
    .title = 切换到瀑布流
    .aria-label = 切换到瀑布流
gallery-scale-toggle =
    .title = 调整图片大小
    .aria-label = 调整图片大小
gallery-scale-label = 图片大小
gallery-scale-range =
    .aria-label = 图片大小
gallery-comparison-rows = 对比维度
gallery-comparison-result-count = { $count } 项结果
gallery-comparison-name-row =
    .title = 命名对比行
    .aria-label = 命名对比行
gallery-comparison-row-label =
    .placeholder = 对比行名称
    .aria-label = 对比行名称
gallery-comparison-add-row =
    .title = 添加对比行
    .aria-label = 添加对比行
gallery-comparison-add-to-group = 添加到分组
gallery-comparison-group-default = 第 { $index } 组
gallery-comparison-no-other-groups = 没有其他分组
gallery-comparison-dissolve-group = 解散分组
gallery-comparison-make-primary = 将 { $tag } 设为主图
gallery-comparison-previous =
    .title = 上一张对比图片
    .aria-label = 上一张对比图片
gallery-comparison-next =
    .title = 下一张对比图片
    .aria-label = 下一张对比图片
gallery-filter-document = 文献
gallery-filter-year = 年份
gallery-filter-collection = 分类
gallery-filter-type = 类型
gallery-filter-keyword = 图注关键词
gallery-filter-keyword-placeholder =
    .placeholder = 搜索图注
gallery-filter-all-documents = 全部文献
gallery-filter-all-years = 全部年份
gallery-filter-all-collections = 全部分类
gallery-filter-all-types = 全部类型
gallery-kind-figure = 图
gallery-kind-table = 表
gallery-kind-formula = 公式
gallery-result-count = { $filtered }/{ $total }
gallery-page = 第 { $page } 页
gallery-loading = 正在加载图表索引...
gallery-load-error = 无法加载图表索引
gallery-api-unavailable = PDF Figure 浏览器 API 不可用
gallery-no-libraries = 没有可用的 Zotero 资料库
gallery-empty-library = 此资料库中没有本地图、表或公式
gallery-empty-filtered = 没有符合当前筛选条件的结果
gallery-image-loading = 正在加载预览...
gallery-image-error = 预览不可用

sidebar-title = Zotero Figure
sidebar-analyze = 解析图、表和公式
sidebar-cancel = 取消解析
sidebar-translate = 翻译图表说明
sidebar-show-original = 显示原文说明
sidebar-expand-caption = 展开说明
sidebar-collapse-caption = 折叠说明
sidebar-translation-error = 部分说明翻译失败，当前显示原文
sidebar-sync-annotations = 将本地结果转换为 Zotero 标注
sidebar-add-all-to-note = 将全部结果保存到笔记
sidebar-clear = 清空本地结果
sidebar-loading = 正在加载本地结果...
sidebar-load-error = 无法加载本地结果
sidebar-analysis-progress-title = 正在解析文档
sidebar-filter-figure =
    { $count ->
       *[other] 图片
    }
sidebar-filter-table =
    { $count ->
       *[other] 表格
    }
sidebar-filter-formula =
    { $count ->
       *[other] 公式
    }
sidebar-refresh = 刷新图、表和公式
sidebar-empty = 暂无生成的图、表或公式
sidebar-start-analysis = 开始解析
sidebar-page = 第 { $page } 页
sidebar-image-preparing = 正在准备图片...
sidebar-image-unavailable = 图片不可用
sidebar-no-caption = 无标题
sidebar-untitled = 未命名图、表或公式
sidebar-menu = 图表公式操作
sidebar-copy-image = 复制图片
sidebar-copy-latex = 复制 LaTeX
sidebar-copy-latex-loading = 正在将公式转换为 LaTeX
sidebar-copy-latex-success = 已复制 LaTeX
sidebar-copy-latex-failed = LaTeX 转换失败：{ $message }
sidebar-rerecognize-latex = 重新识别公式
sidebar-rerecognize-latex-loading = 正在重新识别公式
sidebar-rerecognize-latex-success = 已重新识别公式
sidebar-formula-latex = 公式 LaTeX
sidebar-save-image = 图片另存为...
sidebar-pin-image = 贴图
sidebar-go-to-page = 跳转到页面
sidebar-edit-comment = 修改图注
sidebar-edit-comment-title = 修改结果说明
sidebar-edit-comment-description = 修改保存在本地的图注。保存后会清除这条结果的翻译缓存。
sidebar-edit-comment-save = 保存
sidebar-edit-comment-cancel = 取消
sidebar-edit-latex = 编辑 LaTeX
sidebar-edit-latex-title = 编辑公式 LaTeX
sidebar-edit-latex-description = 编辑本地缓存的 LaTeX 源码。保存后会更新所有已打开的公式视图。
sidebar-edit-latex-failed = 无法打开 LaTeX 编辑器：{ $message }
sidebar-edit-latex-source = LaTeX 源码
sidebar-edit-latex-preview = 预览
sidebar-edit-latex-invalid = 无法渲染此 LaTeX
sidebar-edit-latex-save = 保存
sidebar-edit-latex-cancel = 取消
sidebar-correct-region = 校正区域
sidebar-correct-region-title = 校正结果区域
sidebar-correct-region-selection = 可调整的结果区域
sidebar-correct-region-reset = 恢复检测区域
sidebar-correct-region-save = 保存
sidebar-correct-region-cancel = 取消
sidebar-add-to-note = 添加到笔记
sidebar-remove = 删除本地结果
progress-correct-region = 正在保存校正区域...
progress-correct-region-done = 已保存校正区域
note-title = Zotero Figure 结果

confirm-remove-generated = 是否删除此 PDF 中由 Zotero Figure 生成的全部图、表和公式标注？
confirm-remove-all = 是否删除此 PDF 中的全部标注？这也会删除并非由 Zotero Figure 创建的标注。
confirm-remove-one = 是否删除这个本地结果及其标注镜像？

progress-initializing = 正在初始化版面分析...
progress-exporting = [{ $current }/{ $total }] 正在导出 PDF 页面
progress-detecting = [{ $current }/{ $total }] 正在检测图、表和公式
progress-done = 已保存 { $count } 个本地结果，删除 { $removed } 个，跳过 { $skipped } 个，同步 { $annotations } 个标注，用时 { $seconds } 秒
progress-cancelled = 已取消版面分析
progress-error = 错误：{ $message }
progress-add-note = 正在将结果添加到笔记...
progress-sync-annotations = 正在将本地结果转换为 Zotero 标注...
progress-sync-annotations-done = 已同步 { $created } 个标注，删除 { $removed } 个，跳过 { $skipped } 个
progress-remove-generated = 正在删除生成的图表标注...
progress-remove-all = 正在删除全部标注...
progress-switch-view = 正在切换到 { $view } 视图...

error-model-file-unavailable = 无法准备内置版面模型。请在 Zotero Figure 设置中恢复内置模型。
error-model-integrity = 内置模型的本地副本未通过大小或 SHA-256 校验。请在 Zotero Figure 设置中恢复内置模型。
error-analysis-failed = { $count } 个页面的版面检测失败。请检查模型文件并重启 Zotero。
error-formula-api-key-missing = 请先在 PDF Figure 设置中填写 SiliconFlow API 密钥。
error-formula-api-http = SiliconFlow 返回了 HTTP { $status }。
error-formula-api-network = 无法连接到 SiliconFlow。
error-formula-api-timeout = SiliconFlow 请求超时。
error-formula-api-response = SiliconFlow 未返回有效的 LaTeX。
error-formula-api-failed = 无法完成公式转换。

model-variant-optimized = 内置优化版 Q8 模型

preferences-restore-model = 恢复内置模型
preferences-cancel-install = 取消准备
preferences-status-preparing = 正在准备内置模型...
preferences-status-install-progress = 正在准备 { $loaded } / { $total }（{ $percent }%）
preferences-status-install-cancelled = 已取消准备内置模型
preferences-status-install-failed = 无法准备内置模型：{ $message }
preferences-status-checking = 正在校验模型完整性...
preferences-status-check-failed = 模型校验失败：{ $message }
preferences-status-missing = 尚未准备内置模型副本
preferences-status-invalid = 内置模型副本完整性校验未通过
preferences-status-valid = 已校验：{ $name }
preferences-model-expected = 预期文件：{ $size }，SHA-256 { $hash }
preferences-model-invalid-details = 实际文件：{ $size }，SHA-256 { $hash }
preferences-model-valid-details = { $name }，{ $size }，SHA-256 { $hash }
preferences-api-status-checking = 正在检查所选 SiliconFlow 模型...
preferences-api-status-valid = 密钥检查通过，已可启用公式自动识别。
preferences-api-status-failed = 密钥检查失败：{ $message }
preferences-hash-not-computed = 未计算
preferences-existing-formulae-scanning = 正在查找已有公式图片…
preferences-existing-formulae-progress = 正在转换 { $completed }/{ $total }（成功 { $succeeded }，失败 { $failed }）…
preferences-existing-formulae-complete = 已处理 { $total } 个公式（成功 { $succeeded }，失败 { $failed }）。
preferences-existing-formulae-empty = 所有已识别公式均已有缓存的 LaTeX。
preferences-existing-formulae-failed = 转换失败：{ $message }
preferences-about-version = 版本 { $version }
prompt-figure-caption = 图注释：
